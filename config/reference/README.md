...
**Halo never reads this folder.** It is here to show what the two config files look like without
installing anything.
...
first run, and the Settings app rewrites them click by click, so hand-editing is optional; the file
...
These files were produced by running the v1 → v2 migrator over an existing layout:
...
one particular PC's, not defaults. They are a worked example of the schema, not a layout to copy.
`externalIp.enabled` is shown as `false`, which is what a fresh install gets.

A real config will look different, and that is the point: nothing here is hardware-specific by
design — drive letters, fan channels, GPU indexes and core counts are all discovered at runtime and
never written into config.
